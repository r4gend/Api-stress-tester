...
def render_template(template: Optional[str], request_number: int, user_number: int) -> Optional[str]:
    """Replace placeholders in URL or body with per-request values.

    Supported placeholders: {request_number}, {user_number}, {random}, {timestamp}
    """
    if not template or "{" not in template:
        return template
    return (
        template
        .replace("{request_number}", str(request_number))
        .replace("{user_number}", str(user_number))
        .replace("{random}", uuid.uuid4().hex)
...
class StressTestEngine:
    def __init__(self, test_id: uuid.UUID):
        ...
        # Adaptive Redis write step: ~200 progress updates over the whole test,
        # regardless of scale (min 10). Set once in run() from total_requests.
        self._progress_step = 10
    ...
    async def _make_request(
        ...
        url = render_template(config.target_url, request_number, user_number)
        if config.http_method in (HttpMethod.GET, HttpMethod.HEAD):
            body = None
        else:
            body = render_template(config.body, request_number, user_number)
    ...
    async def _flush_unsaved(self):
        """Persist any results that haven't been written to DB yet.

        Safe to call repeatedly — tracks _save_idx so each row is written exactly once.
        """
        async with self._flush_lock:
            end_idx = len(self.results)
            if self._save_idx >= end_idx:
                return
            batch = self.results[self._save_idx:end_idx]
            async with async_session() as session:
                rows = [
                    TestResult(stress_test_id=self.test_id, **r)
                    for r in batch
                ]
                session.add_all(rows)
                await session.commit()
            self._save_idx = end_idx

    async def _periodic_flush(self, interval: float = 2.0):
        """Background task: flush new results to DB every `interval` seconds."""
        try:
            while True:
                ...
                try:
                    ...
                    # don't kill the flusher on a transient DB hiccup
                    pass
    ...
    async def run(self):
        async with async_session() as session:
            ...
            if not config:
                return
            self._progress_step = max(10, config.total_requests // 200)
            await self._update_status(session, TestStatus.RUNNING, started_at=datetime.utcnow())

        # Background task that streams results to DB every 2s so the
        # frontend timeline endpoint has fresh data during the run.
        ...
        try:
            ...
            # Stop the periodic flusher and persist any final tail
            flush_task.cancel()
            try:
                await flush_task
            except asyncio.CancelledError:
